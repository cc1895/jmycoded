...
const notify = $a.isNode() ? require('./sendNotify') : '';
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $a.isNode() ? require('./jdCookie.js') : '';
...
if ($a.isNode()) {
  ...
  cookiesArr = [$a.getdata('CookieJD'), $a.getdata('CookieJD2'), ...jsonParse($a.getdata('CookiesJD') || "[]").map(item => item.cookie)].filter(item => !!item);
}
let jdNotify = true;//是否开启静默运行。默认true开启
let message = '', subTitle = '';
const JD_API_HOST = 'https://jdjoy.jd.com'
let FEED_NUM = ($a.getdata('joyFeedCount') * 1) || 10;   //喂食数量默认10g,可选 10,20,40,80 , 其他数字不可.

!(async () => {
  if (!cookiesArr[0]) {
    $a.msg($a.name, '【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取', 'https://bean.m.jd.com/bean/signIndex.action', {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});
    return;
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $a.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
      $a.index = i + 1;
      $a.isLogin = true;
      $a.nickName = '';
      await $a.wait(100);
      // await TotalBean();
      // console.log(`\n开始【京东账号${$a.index}】${$a.nickName || $a.UserName}\n`);
      // if (!$a.isLogin) {
      //   $a.msg($a.name, `【提示】cookie已失效`, `京东账号${$a.index} ${$a.nickName || $a.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});
      //
      //   if ($a.isNode()) {
      //     await notify.sendNotify(`${$a.name}cookie已失效 - ${$a.UserName}`, `京东账号${$a.index} ${$a.UserName}\n请重新登录获取cookie`);
      //   }
      //   continue
      // }
      message = '';
      ...
      static jsonp(api, data = {}) {
          return new Promise((resolve, reject) => {
              ...
              const headers = {
                  ...
                  "User-Agent": $a.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($a.getdata('JDUA') ? $a.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1")
                };
...
function showMsg() {
  $a.log(`\n${message}\n`);
  jdNotify = $a.getdata('jdJoyNotify') ? $a.getdata('jdJoyNotify') : jdNotify;
  if (!jdNotify || jdNotify === 'false') {
    //$a.msg($a.name, subTitle, `【京东账号${$a.index}】${$a.UserName}\n` + message);
  }
}
function feedPets(feedNum) {
  return new Promise(resolve => {
    ...
    $a.get(options, async (err, resp, data) => {
      try {
        $a.data = JSON.parse(data);
        if ($a.data.success) {
          if ($a.data.errorCode === 'feed_ok') {
            console.log('喂食成功')
            message += `【喂食成功】${feedNum}g\n`;
          } else if ($a.data.errorCode === 'time_error') {
            console.log('喂食失败：正在食用')
            message += `【喂食失败】您的汪汪正在食用\n`;
          } else if ($a.data.errorCode === 'food_insufficient') {
            ...
            console.log(`其他状态${$a.data.errorCode}`)
          }
        }
      } catch (e) {
        $a.logErr(e, resp);
...
function validate() {
  return new Promise(resolve => {
    let opt = {
      url: `//jdjoy.jd.com/common/pet/getPetTaskConfig?validate=${$a.validate}&reqSource=h5&invokeKey=NRp8OPxZMFXmGkaE`,
      method: "GET",
      data: {},
      credentials: "include",
      header: {"content-type": "application/json"}
...
function jsonParse(str) {
  if (typeof str == "string") {
    try {
      ...
      console.log(e);
      $a.msg($a.name, '', '请勿随意在BoxJs输入框修改内容\n建议通过脚本去获取cookie')
      return [];
    }
  }
}
function TotalBean() {
  return new Promise(async resolve => {
    ...
    $a.post(options, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$a.name} API请求失败，请检查网路重试`)
        } else {
          if (data) {
            data = JSON.parse(data);
            if (data['retcode'] === 13) {
              $a.isLogin = false; //cookie过期
              return
            }
            if (data['retcode'] === 0) {
              $a.nickName = (data['base'] && data['base'].nickname) || $a.UserName;
            } else {
